...
class StructureSet:
    ...
    :param structures: the structures that will make up the StructureSet."""

    def __init__(self, structures):
        self._d = {}
        for structure in structures:
            if structure.id in self._d:
                self._d[structure.id].add(structure)
            else:
                self._d[structure.id] = {structure}
...
def query(func, tuple_=False):
    ...
    :param bool tuple_: if ``True``, objects will be returned in a tuple.
    :rtype: ``function``"""
