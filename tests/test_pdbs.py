...
class AlphaHelixTests(PdbTest):

    def setUp(self):
        self.helix = Pdb(PdbDataFile(PdbFile(
         "HELIX    1  HA VAL A   11  MET A   12  1                                   2\n"
         "HELIX    2  HB VAL B 1011  MET B 1012  5                                   2\n"
         "ATOM      1  N   VAL A  11       3.696  33.898  63.219  1.00 21.50           N\n"
         "ATOM      2  CA  VAL A  11       3.198  33.218  61.983  1.00 19.76           C\n"
         "ATOM      3  C   VAL A  11       3.914  31.863  61.818  1.00 19.29           C\n"
         "ATOM      4  O   VAL A  11       5.132  31.792  61.932  1.00 19.78           O\n"
    ...
    def test_can_parse_alpha_helices(self):
        a = self.helix.model.get_chain_by_id("A")
        b = self.helix.model.get_chain_by_id("B")
        self.assertEqual(len(a.alpha_helices), 1)
        self.assertEqual(len(b.alpha_helices), 1)
        self.assertIsInstance(list(a.alpha_helices)[0], PdbAlphaHelix)
        self.assertIsInstance(list(b.alpha_helices)[0], PdbAlphaHelix)
        self.assertIsNot(list(a.alpha_helices)[0], list(b.alpha_helices)[0])


    def test_alpha_helices_are_correct(self):
        helix_a = list(self.helix.model.get_chain_by_id("A").alpha_helices)[0]
        helix_b = list(self.helix.model.get_chain_by_id("B").alpha_helices)[0]
        self.assertEqual(helix_a.helix_id, "HA")
        self.assertEqual(len(helix_a.residues), 2)
        self.assertEqual(helix_a.helix_class, "Right-handed alpha")
