...
class StructureExtractionTests(TestCase):
    ...
    def test_can_extract_structure_from_pdb_file(self, mock_conect, mock_atom):
        ...
        pdb_file.records.side_effect = [atom_recs, hetatom_recs, conect_recs]
        ...
        mock_atom.assert_any_call(atom_recs[0])
        mock_atom.assert_any_call(atom_recs[1])
        mock_atom.assert_any_call(hetatom_recs[0])
        mock_atom.assert_any_call(hetatom_recs[1])
...
class AtomRecordToAtomDictTests(TestCase):
    ...
    def setUp(self):
        self.record = PdbRecord("ATOM    107  N   GLY A  13A     12.681  " +
         "37.302 -25.211 1.000 15.56           N-2") # Can't really mock :(


    def test_can_convert_atom_record(self):
